<?php
    class EntradasModel extends Mysql
    {
        ...
        public function NombreProveedor(string $nombre){
        ...
        public function buscarProducto(string $nombre){
            $sql = "SELECT * FROM producto WHERE nombre LIKE '%$nombre%' OR codigo LIKE '%$nombre%' AND status = 1 ";
            $request = $this->select_all($sql);
            return $request;
        }
        ...
        public function registrarDetalle(int $idproducto,int $idusuario,int $cantidad,float $precio,float $subtotal){
            $sql = "INSERT INTO dtmp_entrada (productoid,usuarioid,cantidad,precio,subtotal) VALUES (?,?,?,?,?)";
        ...
        public function getHistorialEntrada(){
			$sql = "SELECT e.identrada,e.total,e.datecreated,e.timecreated,e.usuarioid,e.status,e.proveedorid,p.nombre as proveedor,u.username as usuario FROM entradas e 
            INNER JOIN usuario u ON e.usuarioid = u.idusuario INNER JOIN proveedor p ON e.proveedorid = p.idproveedor";
        ...
        public function getProveedorEntrada(int $identrada){
            $sql="SELECT e.*,p.ruc,p.nombre as proveedor, p.telefono,p.direccion FROM entradas e INNER JOIN proveedor p
